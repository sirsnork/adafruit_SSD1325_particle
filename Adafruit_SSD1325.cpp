...
void Adafruit_SSD1325::off(void) {
  command(SSD1325_DISPLAYALLOFF);
}

void Adafruit_SSD1325::on(void) {
  command(SSD1325_SETCLOCK); /* set osc division */
  command(0xF1); /* 145 */
  command(SSD1325_SETMULTIPLEX ); /* multiplex ratio */
  command(0x3f); /* duty = 1/64 */
...
void Adafruit_SSD1325::command(uint8_t c) {
  ...
  if (sclk == -1) {
#ifdef SPI_HAS_TRANSACTION
    SPI1.beginTransaction(oledspi);
#else
    SPI1.setDataMode(SPI_MODE0);
    SPI1.setClockDivider(ADAFRUIT_SSD1325_SPI);
#endif
  }
...
#ifdef SPI_HAS_TRANSACTION
  if (sclk == -1)
      SPI1.endTransaction();              // release the SPI bus
#endif
}
...
#ifdef SPI_HAS_TRANSACTION
  if (sclk == -1)
     SPI1.endTransaction();              // release the SPI buss
#endif
}
